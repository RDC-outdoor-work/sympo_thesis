...
\subsubsection{セマンティックセグメンテーションを用いたロボットの制御}
% 本チームは, セグメンテーションを用いて領域分割を行う. また, 
領域分割した画像から走行可能領域(道)を検出し, その結果に基づいて行動生成を行う. 本節では, セマンティックセグメンテーションを用いたロボットの制御について, 一連の流れを紹介する. 

\begin{enumerate}
  \item セマンティックセグメンテーションの適用\\
