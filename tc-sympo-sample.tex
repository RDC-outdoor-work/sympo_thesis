...
これらの処理を施した画像を用いて, \figref{fig:seg}の対応する箇所に応じた行動を生成する. 具体的には, \figref{fig:seg}のTurn leftの範囲に走行可能領域以外のラベルによるピクセル数がある程度多くなると, 左に曲がるヨー方向の角速度を出力する. また, GitHubのdeeplabv3\_plus\_pytorch\_rosでプログラムとデータセットを公開している.
